...
public class RiskCacheEvictor {
     ...
     * score, the derived top-N ranking, and the fleet CVE listing. {@code feedsStatus} is left
     ...
     * and the feeds-status corpus snapshot (so {@code EmptyCorpusBanner} flips on the next poll).
     */
    @Caching(evict = {
        @CacheEvict(cacheNames = CacheNames.DEVICE_RISK, allEntries = true),
        @CacheEvict(cacheNames = CacheNames.TOP_RISK, allEntries = true),
