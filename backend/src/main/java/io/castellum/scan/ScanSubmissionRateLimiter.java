...
/**
 * Per-actor sliding-window rate limiter for {@code POST /api/scan}.
 *
 * <p>Mirrors {@link io.castellum.security.LoginRateLimiter} structurally —
 * separate bean so its window/budget can be tuned independently. Keyed by the
 * authenticated username, not the source IP: the threat model is a single
 * user (legitimately or not) saturating the {@code scanTaskExecutor} queue,
 * not credential stuffing.
 *
 * <p>Defaults: 20 submissions / 60-minute window. Tunable via
 * {@code castellum.security.rate-limit.scan-window-seconds} and
 * {@code castellum.security.rate-limit.scan-max-attempts}.
 */
@Component
public class ScanSubmissionRateLimiter {
    ...
    public ScanSubmissionRateLimiter(
            @Value("${castellum.security.rate-limit.scan-window-seconds:3600}") long windowSeconds,
            @Value("${castellum.security.rate-limit.scan-max-attempts:20}") int maxAttempts,
            Clock clock) {
    ...
    /** Returns true if this submission is permitted; false if it would exceed the budget. */
    ...
    /** Record an accepted submission against the budget. */
    public synchronized void recordSubmission(String actor) {
        prune(actor);
